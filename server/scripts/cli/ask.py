...
from src.core.logging_utils import log_interaction, build_retrieval_log
...
def main() -> None:
    ...
    # Try to derive a numeric confidence from the top TF-IDF score if available
    numeric_conf = None
    if raw_results:
        top = raw_results[0]
        score = getattr(top, "score", None)
        if isinstance(score, (int, float)):
            # Convert similarity in [0, 1] to a percentage
            numeric_conf = int(round(score * 100))

    print("\n=== Confidence ===")
    if numeric_conf is not None:
        print(f"{ans.confidence.capitalize()} ({numeric_conf}/100)")
    else:
